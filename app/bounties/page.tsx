...
export default async function BountiesPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
    ...
    const catIcon: Record<string, string> = { Research: '🔍', Delivery: '📦', Creative: '🎨', Tech: '💻', default: '🎯' };
        ...
        <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
            <div className="wrap" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1.5rem', borderBottom: '1px solid var(--border)', marginBottom: '1.5rem' }}>
                    {tabs.map(t => (
                        <Link key={t} href={`/bounties?tab=${t}`}
                            style={{
                                fontFamily: 'var(--font-mono)', fontSize: '0.8125rem', paddingBottom: '0.75rem', borderBottom: tab === t ? '2px solid var(--orange)' : '2px solid transparent',
                                color: tab === t ? 'var(--text)' : 'var(--text-3)', fontWeight: tab === t ? 600 : 400, textDecoration: 'none', transition: 'color 0.15s', whiteSpace: 'nowrap'
                            }}>
                            {t}
                        </Link>
                    ))}
                ...
                <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start' }}>
                    ...
                    <aside style={{ width: '11rem', flexShrink: 0 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                            <div>
                                <label className="mono" style={{ display: 'block', fontSize: '0.65rem', color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>Category</label>
                                <select className="inp" style={{ width: '100%', fontSize: '0.8rem' }}>
                                    <option>All Categories</option>
                                    <option>Research</option>
                                    <option>Creative</option>
                                    <option>Physical Proxy</option>
                                    <option>Delivery</option>
                                </select>
                            </div>
                            <div>
                                <label className="mono" style={{ display: 'block', fontSize: '0.65rem', color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>Skill Needed</label>
                                <input type="text" placeholder="e.g. photography" className="inp" style={{ width: '100%', fontSize: '0.8rem' }} />
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.375rem' }}>
                                <div>
                                    <label className="mono" style={{ display: 'block', fontSize: '0.65rem', color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>Min $</label>
                                    <input type="number" placeholder="0" className="inp" style={{ width: '100%', fontSize: '0.8rem' }} />
                                </div>
                                <div>
                                    <label className="mono" style={{ display: 'block', fontSize: '0.65rem', color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>Max $</label>
                                    <input type="number" placeholder="∞" className="inp" style={{ width: '100%', fontSize: '0.8rem' }} />
                                </div>
                            </div>
                        </div>
                    </aside>
                    ...
                    {/* List */}
                    <main style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {bounties.length === 0 ? (
